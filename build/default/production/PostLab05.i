...
RESET_TMR0 MACRO TMR_VAR ; FUNCIÓN QUE RESETEA EL TMR0 CON CUALQUIER VALOR
    ...
    BCF ((INTCON) and 07Fh), 2 ; LIMPIA BANDERA DE INTERRUPCIÓN
    ENDM
...
CONFIG_IO:
    ...
    BCF PORTB, 0 ; ((PORTB) and 07Fh), 0 EN 0
    BCF PORTB, 1 ; ((PORTB) and 07Fh), 1 EN 0
    MOVLW 0xFF
    MOVWF PORTC ; PRENDE PORTC (APAGA DISPLAY ÁNODO COMÚN)
    BSF PORTD, 0 ; ((PORTB) and 07Fh), 0 EN 1
    BSF PORTD, 1 ; ((PORTB) and 07Fh), 1 EN 1
    BSF PORTD, 2 ; ((PORTB) and 07Fh), 1 EN 1 (APAGA DISPLAY ÁNODO COMÚN)
    RETURN

CONFIG_RELOJ:
    BANKSEL OSCCON
    BCF ((OSCCON) and 07Fh), 0 ; RELOJ INTERNO
    BSF ((OSCCON) and 07Fh), 6 ; ((OSCCON) and 07Fh), 6
    BSF ((OSCCON) and 07Fh), 5 ; ((OSCCON) and 07Fh), 5
    BCF ((OSCCON) and 07Fh), 4 ; ((OSCCON) and 07Fh), 4 -> IRCF<2:0> = 110 -> 4 MHz
    RETURN

CONFIG_TMR0:
    BANKSEL OPTION_REG
    BCF ((OPTION_REG) and 07Fh), 5 ; INCREMENTO CON PULSOS DE RELOJ INTERNO
    BCF ((OPTION_REG) and 07Fh), 3 ; PRESCALER A TMR0
    BSF ((OPTION_REG) and 07Fh), 2
    BSF ((OPTION_REG) and 07Fh), 1
    BSF ((OPTION_REG) and 07Fh), 0 ; PRESCALER 1 : 256
    RESET_TMR0 217
    RETURN

CONFIG_INT:
    BANKSEL INTCON
    BSF ((INTCON) and 07Fh), 7 ; INTERRUPCIONES GLOBALES HABILITADAS
    BSF ((INTCON) and 07Fh), 5 ; INTERRUPCIÓN TMR0 HABILITADA
    BCF ((INTCON) and 07Fh), 2 ; LIMPIA BANDERA DE DESBORDAMIENTO DE TMR0
    RETURN
...
EVALUAR_CENTENAS:
    ...
    BTFSS STATUS, 0 ; SI NO HAY ((STATUS) and 07Fh), 0, RESTA DA NEGATIVO Y SALE DE SUBRUTINA
    RETURN
...
EVALUAR_DECENAS:
    ...
    BTFSS STATUS, 0 ; SI NO HAY ((STATUS) and 07Fh), 0, RESTA DA NEGATIVO Y SALE SE SUBRUTINA
    RETURN
...
MOSTRAR_VALOR:
 ...
 BCF PORTD, 2 ; APAGA ((PORTD) and 07Fh), 2 PRENDIENDO DISPLAY 1
 ...
 MOVF DISPLAY+1, W
 MOVWF PORTC ; COLOCA EL BYTE DE DISPLAY EN PUERTO C
 BCF PORTD, 1 ; APAGA ((PORTD) and 07Fh), 1 PREDIENDO DISPLAY 2
 ...
 MOVF DISPLAY+2, W
 MOVWF PORTC ; COLOCA EL BYTE DE DISPLAY EN PUERTO C
 BCF PORTD, 0 ; APAGA ((PORTD) and 07Fh), 0 PRENDIENDO DISPLAY 3
